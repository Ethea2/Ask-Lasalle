...
const PostcardFull = ({ post, user }) => {
    return ( <>
        <div className="postcard" class="h-1/3 bg-lasalle-white flex rounded-2xl m-auto mb-4">
            ...
            <div className="post-container" class="w-full p-4 flex flex-col justify-between">
                ...
                <hr class="bg-neutral-500 mx-2 h-0.5"></hr>

                <div className="post-content" class="p-2 mt-2 flex flex-col justify-between overflow-hidden">
                    <div className="post-preview" class="h-2/4">
                        <Link to={'/viewpost/' + post.postid} class="text-2xl font-bold">
                            <p>{post.title}</p>
                        </Link>
                        <p class="mt-4 mb-6">{post.body}</p>
                    </div>
